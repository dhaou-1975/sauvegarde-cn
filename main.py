...
class CNCApplication(tk.Tk):
    ...
    def show_about(self):
        about_text = (
            ...
            f"Logiciel de gestion de programmes d'usinage, de traçabilité des OF,\n"
            f"de sauvegarde et de transfert RS232 pour commandes numériques NUM 1060."
        )
    ...
    def setup_compare_tab(self, parent):
        ...
        self.tree_comp.heading("model", text="Nom de model")
        self.tree_comp.heading("fichier", text="Nom Prog. (Fichier/Chemin)")
        ...
        btn_print = tk.Button(frame_bottom, text="🖨️ Imprimer Rapport Comparaison", 
                              bg="#424242", fg="white", font=("Arial", 9, "bold"),
                              command=lambda: self.print_treeview_data(self.tree_comp, "Rapport de Comparaison de Dossiers"))
    ...
    def on_recurrence_change(self, event=None):
        ...
        if rec_type == "Mensuel":
            ...
            for chk in self.days_checkbuttons:
                chk.config(state=tk.NORMAL if rec_type == "Hebdomadaire" else tk.NORMAL)
    ...
    def setup_of_tab(self, parent):
        frame_new = ttk.LabelFrame(parent, text=" Saisie de l'Ordre de Fabrication & Traçabilité Brut ")
    ...
    def create_of_auto(self):
        ...
        cursor.execute('''
            INSERT INTO work_orders (of_number, machine, assigned_operator, priority)
            VALUES (?, ?, ?, ?)
        ''', (of_num, self.combo_of_mach.get(), self.current_user['username'], self.combo_of_prio.get()))

        cursor.execute('''
            INSERT INTO work_order_items (of_number, model_name, prog_name, block_num, block_density, pain_num, pain_weight)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        ''', (of_num, model_name, prog, b_num, b_dens, p_num, p_weight))
    ...
    def load_of_data(self):
        ...
        cursor = conn.cursor()
        cursor.execute('''
            SELECT o.id, o.of_number, i.model_name, i.prog_name, o.machine, o.assigned_operator, o.priority, 
                   i.block_num, i.block_density, i.pain_num, i.pain_weight, o.status, o.created_at
            FROM work_orders o
            LEFT JOIN work_order_items i ON o.of_number = i.of_number
            ORDER BY o.id DESC
        ''')
        for row in cursor.fetchall():
