...
class ProgrammeCNCManager(tk.Tk):
    ...
    # ==========================================
    # ONGLET 1 : LISTE PROGRAMME USINAGE
    # ==========================================
    def setup_mapping_tab(self):
        ...
        ttk.Label(frame_top, text="Importer/Mettre à jour à partir d'un fichier CSV (séparateur point-virgule) :").pack(side=tk.LEFT, padx=10, pady=10)
        
        btn_import = tk.Button(frame_top, text="📥 Importer Fichier (CSV)", font=("Arial", 9, "bold"), bg="#0288D1", fg="white", command=self.import_mapping_file)
    ...
    def import_mapping_file(self):
        file_path = filedialog.askopenfilename(filetypes=[("Fichiers CSV", "*.csv"), ("Tous", "*.*")])
        if not file_path:
            return
    ...
    # ==========================================
    # ONGLET 3 : CONFIGURATION & SAUVEGARDE
    # ==========================================
    ...
    def run_task_manual(self):
        ...
        if not os.path.exists(src):
            messagebox.showerror("Erreur Source", f"Le dossier source n'existe pas :\n{src}")
            return
